...
class SparkConf(loadDefaults: Boolean) extends Cloneable with Logging with Serializable {
  ...
  private[spark] def loadFromSystemProperties(silent: Boolean): SparkConf = {
    ...
    // 读取System.getProperties中以spark.作为前缀的那部分属性
  ...
  private[spark] def set(key: String, value: String, silent: Boolean): SparkConf = {
    ...
    // SparkConf类初始化时，这里为true
    // 检查key是否过时了，如果过时了，给一个warn级别的log
    if (!silent) {
      logDeprecationWarning(key)
    }
    // 从系统中读取到的配置文件放入setting
...
private[spark] object SparkConf extends Logging {
   ...
   * 如果来的配置key是过去的，这里给一个warn的log级别提示
   * 1.过时配置中找到，将来可能会移除。提示并return
   * 2.可替换配置中找到，提示你输入的key是过时的，可以更换成新的，并return
   * 3.如果是spark.akka和spark.ssl.akka为前缀的，提示2.0后不在使用了
   */
